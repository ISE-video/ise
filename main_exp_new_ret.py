...
def get_env(task, seed, identity):
    ...
    if task in ['push-test-v2-goal-observable', 'pick-bar-v2-goal-observable']:
        ...
        env = env_dict[task](seed=seed, init_seed=seed, cm_sample_seed=seed, letter=alphabet, cm_visible=False, font_size=10, font_path="./project/alphabet_dataset_gen/basic.ttf")
    else:
        raise NotImplementedError
    return env
...
class ExplicitRetrievalModule():
    def __init__(self, task_name, training_seeds=10, max_training_offset=36, on=True, prob_based=False, temperature=40, pca=False, enc_method="clip", guidance=0.0, random_generation=0.0):
        ...
        if on:
            ...
            with open(f'./down_dataset/{task_name}/features.npy', 'rb') as f:
                self.features = np.load(f)
                print("FEATURES", self.features.shape)

            with open(f'./down_dataset/{task_name}/dataset.json', 'r') as f:
        ...
        if self.pca:
            ...
            projected = self.pca.transform(self.features)
    ...
    def retrieve_nearest_n(self, feat, n=1):
        ...
        if not self.prob_based:
            ...
            dist = torch.cdist(feat.unsqueeze(0), self.features, p=2).squeeze()
            nearest_idx = torch.argsort(dist)[:n]
            filename = self.files[nearest_idx]
            print(filename)
            assert False
        else:
            ...
            embeddings = [self.features[i] for i in new_indices]

            embeddings_tensor = torch.stack([torch.tensor(embedding) for embedding in embeddings])

            # print("EMBEDDING", len(embeddings), embeddings[0].shape)    
...
def main(seed, n, retrieve_on, retrieval_module, out_file, out_dir, agg_metric, temperature, pca, task_name, encode_method, save_video, refine_steps, refine_from_scratch, dist_metric, guidance, random_generation):
    ...
    if task_name == 'push_bar':
        ...
        alphabet = ALP_LIST[seed % len(ALP_LIST)]
        env = env_dict[task](seed=seed, axis_seed=seed, alphabet=alphabet, cm_visible=False, font_size=10, font_path="./project/alphabet_dataset_gen/basic.ttf")
